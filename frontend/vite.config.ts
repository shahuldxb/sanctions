import { defineConfig } from 'vite'
import react from '@vitejs/plugin-react'
import path from 'path'

export default defineConfig({
  plugins: [react()],
  resolve: {
    alias: {
      '@': path.resolve(__dirname, './src'),
    },
  },
  build: {
    // Emit manifest.json so the server can map logical names to hashed filenames
    manifest: true,
    chunkSizeWarningLimit: 1000,
    // NOTE: Do NOT use manualChunks here — it causes "Cannot access before initialization"
    // TDZ errors due to circular dependencies in the codebase. Let Vite handle chunking automatically.
  },
  server: {
    host: '0.0.0.0',
    port: 3000,
    proxy: {
      '/api': { target: 'http://localhost:5000', changeOrigin: true },
      '/scraper': { target: 'http://localhost:5002', changeOrigin: true, rewrite: (p) => p.replace(/^\/scraper/, '') },
      '/stream': { target: 'http://localhost:5002', changeOrigin: true }
    }
  }
})
